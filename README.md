...
To load the docs into Chroma DB, run `python3 django_docs.py load`. This may well take quite a while, depending on the
speed of your setup.

To run a search directly against the Chroma DB, run `python3 django_docs.py search`.

To ask a query of deepseek, taking into account context from Chroma DB, run `python3 django_docs.py ask`.

To compare the output of this setup with a raw query, ask the same question (make sure you mention it's for the version
of Django you're building docs for to ollama directly.

## Motivation

The goal of this project is to be able to use AI in a local-only set up with minimal resources on a very specific data
set (in this case, the Django documentation).
